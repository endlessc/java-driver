 ...
 * long countById(int id, int year);
 * </pre>
 *
 * <h3>Return type</h3>
 *
 ...
 *       ${qualifiedTableId} uses the keyspace if it is available, but resolves to the table name
 *       only if it isn't. Whether the query succeeds or not depends on whether the {@link Session}
 *       that the mapper was built with has a {@linkplain SessionBuilder#withKeyspace(CqlIdentifier)
 *       default keyspace}.
 * </ul>
 */
